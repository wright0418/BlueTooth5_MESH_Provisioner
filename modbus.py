...
class ModbusRTU:
    ...
    def bytes_to_hex(self, data):
        """
        將 bytes 數據轉換為十六進制字符串，用於調試顯示
        
        Args:
            data (bytes): 要轉換的數據
            
        Returns:
            str: 十六進制字符串，如: "01 03 00 00 00 0A C5 CD"
        """
        return ' '.join(f'{b:02X}' for b in data)

def test_modbus_rtu():
    """
    簡單測試 ModbusRTU 類別的各項功能
    """
    print("測試 MODBUS RTU 類別...")
    ...
    # 測試生成封包
    print("\n1. 測試生成讀取保持寄存器請求封包:")
    packet1 = modbus.read_holding_registers_request(slave_address=1, start_address=100, quantity=5)
    print(f"封包 (HEX): {modbus.bytes_to_hex(packet1)}")
    print(f"封包 (Bytes): {packet1}")
    
    print("\n2. 測試生成寫入單個寄存器請求封包:")
    packet2 = modbus.write_single_register_request(slave_address=1, register_address=200, register_value=12345)
    print(f"封包 (HEX): {modbus.bytes_to_hex(packet2)}")
    
    print("\n3. 測試生成寫入多個寄存器請求封包:")
    register_values = [10, 20, 30, 40, 50]
    packet3 = modbus.write_multiple_registers_request(slave_address=1, start_address=300, register_values=register_values)
    print(f"封包 (HEX): {modbus.bytes_to_hex(packet3)}")

    print("\n4. 測試生成讀取線圈請求封包:")
    packet4 = modbus.read_coils_request(slave_address=1, start_address=0, quantity=16)
    print(f"封包 (HEX): {modbus.bytes_to_hex(packet4)}")
    
    # 測試解析封包
    print("\n5. 測試解析封包:")
    # 模擬一個讀取保持寄存器的回覆封包 (從站地址 1, 功能碼 3, 10 bytes 數據, 5 個寄存器)
    # 01 03 0A 00 0A 00 14 00 1E 00 28 00 32 XX XX (CRC位置用 XX 表示，實際計算)
    response_data = bytes([0x01, 0x03, 0x0A, 0x00, 0x0A, 0x00, 0x14, 0x00, 0x1E, 0x00, 0x28, 0x00, 0x32])
    # 添加 CRC
    crc = modbus._calculate_crc(response_data)
    response_data += crc
    
    print(f"模擬的回覆封包: {modbus.bytes_to_hex(response_data)}")
    parsed = modbus.parse_rtu_packet(response_data)
    if parsed:
        print(f"解析結果: 地址={parsed['slave_address']}, 功能碼={parsed['function_code']}, "
              f"數據長度={len(parsed['data'])}, 是否異常={parsed['is_exception']}")
              
        registers = modbus.read_holding_registers_response(response_data)
        if registers:
            print(f"寄存器值: {registers}")
    
    # 測試異常回覆
    print("\n6. 測試異常回覆:")
    # 模擬一個異常回覆封包 (從站地址 1, 功能碼 0x83, 異常碼 0x02)
    exception_data = bytes([0x01, 0x83, 0x02])
    # 添加 CRC
    crc = modbus._calculate_crc(exception_data)
    exception_data += crc
    
    print(f"異常回覆封包: {modbus.bytes_to_hex(exception_data)}")
    parsed = modbus.parse_rtu_packet(exception_data)
    if parsed and parsed['is_exception']:
        print(f"解析結果: 地址={parsed['slave_address']}, 功能碼={parsed['function_code']}, "
              f"異常碼={parsed['exception_code']}")
        print(f"異常信息: {modbus.get_exception_message(parsed['exception_code'])}")
